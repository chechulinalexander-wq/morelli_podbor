...
from door_analyzer import DoorAnalyzer, InvalidImageError
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        print(f'[BOT] Invalid image: {str(e)}')
        error_message = f"""❌ {str(e)}

Пожалуйста, отправьте фотографию двери (входной или межкомнатной).

Примеры подходящих фото:
✅ Фото входной двери
✅ Фото межкомнатной двери
✅ Дверное полотно целиком

Не подходят:
❌ Окна, мебель
❌ Фото других предметов
❌ Скриншоты или текст"""
...
Найдено {len(recommendations)} лучших вариантов:"""
    ...
    for i, handle in enumerate(recommendations, 1):
        ...
        try:
            ...
            image_url = handle.get('image_url')
            if image_url:
                await update.message.reply_photo(photo=image_url, caption=message)
                print(f'[BOT] Sent photo from URL: {image_url}')
            else:
                # Fallback to text if no image URL
                await update.message.reply_text(message)
                print(f'[BOT] No image URL for handle: {handle.get("name")}')
        except Exception as e:
